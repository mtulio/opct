...
and check the logs for errors.`,
	...
	checkSum.Checks = append(checkSum.Checks, &Check{
		...
		Test: func() CheckResult {
			res := CheckResult{Name: CheckResultNameFail, Target: "<=12H,W>12H,F>24H"}
			res.Actual = "N/A"
			if re.Provider == nil || re.Provider.Version == nil || re.Provider.Version.OpenShift == nil {
				return res
			}
			...
			// get install completion time
			start, ert = time.Parse(layout, re.Provider.Version.OpenShift.HistoryCompletionTime)
			if ert != nil {
				log.Warnf("Failed to parse install time: %v", ert)
				return res
			}

			// get conformance test completion time
			for _, rti := range re.Provider.Runtime.ServerLogs {
				if rti.Name == "server finished" {
					end, ert = time.Parse(time.RFC3339, rti.Time)
					if ert != nil {
						log.Warnf("Failed to parse server finished time: %v", ert)
						return res
					}
					break
				}
			...
			// get the diff from those timers
			diff := end.Sub(start)
			res.Actual = fmt.Sprintf("%.2f Hrs", diff.Hours())

			// Must not accept older than 1 day
			if diff.Hours() > 24.0 {
				res.Name = CheckResultNameFail
				return res
			}

			// Must tolerate and notify older than 12h
			if diff.Hours() > 12.0 {
				res.Actual = fmt.Sprintf("%.2f Hrs", diff.Hours())
				res.Name = CheckResultNameWarn
				return res
